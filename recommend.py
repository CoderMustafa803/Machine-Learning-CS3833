...
def postprocess_movies(title2movie):
  ...
  for _, movie in title2movie.items(): # movie is a dict
    for f,v in movie.items():
    ...
    if 'demographic' in movie.keys(): # extract gender_feat, nonus_feat and age_feat from demographic info
      ...
      if demographic_keys != sorted(dems.keys()):
        ...
        continue # don't add
      votes = {k: float(v[0]) for k,v in dems.items()}
      ratings = {k: v[1] for k,v in dems.items()}
      ...
      ages_votes = sorted([(a, votes[a]) for a in age_brackets], key=lambda x: x[1])
...
def create_vectors(title2movie, feat2items):
  ...
  for key,movie in title2movie.items():
...
def get_normed_vecs(title2MVec):
  ...
  # Get the first value from the dict_values object and extract the dimension
  d = list(title2MVec.values())[0].dim if len(title2MVec) > 0 else 0

# Get the total number of movies by finding the length of the dictionary keys
  num_movies = len(title2MVec)
  means = get_means(d, title2MVec, num_movies)
  stds = get_stds(d, title2MVec, num_movies, means)

  # Normalize the movie vectors
  for _,mvec in title2MVec.items():
...
def get_dists(query_title, title2MVec_reweighted, distance_function=manh_dist):
  ...
  for title,mvec in title2MVec_reweighted.items():
...
def get_feature_order(feat2weight):
  ...
  feat_wt_tuple_list = sorted(feat2weight.items(), key=lambda x:x[1],reverse=True) # sort by weight
...
def reweight(title2MVec_norm, feat2weight):
  ...
  for feature,weight in feat2weight.items():
...
def get_score(feat2weight, title2MVec_norm, title2movie, distance_function=eucl_dist):
  ...
  movie_rec_rank_tuples = sorted(movie_rec_rank_tuples, key=lambda x: x[2], reverse=True)
